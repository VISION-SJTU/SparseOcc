...
class SparseMask2FormerOccHead(MaskFormerHead):
    ...
    def forward(self, 
        ...
        # FG_THRESHOLD = 0.4
        # coarse_mask = coarse_occ[:, 1:, :, :, :].sigmoid() > FG_THRESHOLD
        # fg_mask = coarse_mask.max(dim=1)[0]  # B, H, W, D
        # nonzero_index = fg_mask.int().nonzero()
        # nonempty_coords = nonzero_index.type(torch.long).to(coarse_occ.device)
        # assert nonempty_coords.shape[0] > 0, 'no foreground in coarse voxel'
        
        # nonempty_feats = mask_features[nonempty_coords[:, 0], :, nonempty_coords[:, 1], nonempty_coords[:, 2], nonempty_coords[:, 3]].squeeze()
